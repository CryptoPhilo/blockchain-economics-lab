#!/usr/bin/env python3
"""
DEPRECATED legacy draft watcher — do not run in production.

This archived watcher scans the obsolete GDrive drafts/{ECON,MAT,FOR}/ folders.
Current operations use BCE Research Source Drafts as source material and publish
slide PDFs through scripts/pipeline/watch_slides.py. The legacy watcher exits
before any Drive scan unless ALLOW_LEGACY_DRAFT_WATCHER=1 is set for historical
reproduction.

Usage:
    ALLOW_LEGACY_DRAFT_WATCHER=1 python watch_drafts.py --type for --dry-run
"""

import argparse
import os
import subprocess
import sys
import time
from datetime import datetime, timezone
from pathlib import Path
from typing import List, Dict

sys.path.insert(0, str(Path(__file__).resolve().parent))

try:
    from pipeline_env import bootstrap_environment
except ModuleNotFoundError:
    def bootstrap_environment():
        return None

bootstrap_environment()

os.environ.setdefault('GDRIVE_ROOT_FOLDER_ID', '')

REPORT_TYPES = ['econ', 'mat', 'for']
LOG_DIR = Path(__file__).resolve().parent.parent.parent / "logs" / "pipeline"
LOG_DIR.mkdir(parents=True, exist_ok=True)
ALLOW_LEGACY_ENV = 'ALLOW_LEGACY_DRAFT_WATCHER'
CURRENT_PIPELINE_GUIDANCE = (
    "This legacy watcher is disabled. It scans obsolete drafts/{ECON,MAT,FOR} "
    "folders and must not be used for current operations.\n"
    "Current source path: GDrive 'BCE Research Source Drafts' "
    "(example: shiba-inu_econ_v1_en.md).\n"
    "Current publish watcher: python scripts/pipeline/watch_slides.py "
    "--type econ --slug shiba-inu --dry-run\n"
    f"Historical override only: {ALLOW_LEGACY_ENV}=1"
)


def legacy_override_enabled() -> bool:
    return os.environ.get(ALLOW_LEGACY_ENV) == '1'


def guard_legacy_watcher() -> bool:
    if legacy_override_enabled():
        return True
    print(CURRENT_PIPELINE_GUIDANCE, file=sys.stderr)
    return False

def scan_type(report_type: str, slug: str = None, force: bool = False) -> List[Dict]:
    from ingest_report import scan_drafts
    return scan_drafts(report_type, filter_slug=slug, force=force)


def process_type(report_type: str, dry_run: bool = False, slug: str = None, force: bool = False) -> tuple[int, int]:
    ingest_script = Path(__file__).parent / 'ingest_report.py'
    cmd = [sys.executable, str(ingest_script), '--type', report_type]
    if dry_run:
        cmd.append('--dry-run')
    if slug:
        cmd.extend(['--slug', slug])
    if force:
        cmd.append('--force')

    print(f"\n  Executing: python ingest_report.py --type {report_type}" +
          (f" --slug {slug}" if slug else "") +
          (" --force" if force else "") +
          (" --dry-run" if dry_run else ""))

    # Stream subprocess stdout/stderr line-by-line so GitHub Actions logs reflect
    # real-time progress. Previously stdout was captured silently for the entire
    # 4-hour run, which made external cancels (BCE-1047 case: 24min cancel)
    # impossible to diagnose because no slug/stage trace ever reached the workflow log.
    timeout_secs = 14400  # 4 hour soft cap; workflow timeout-minutes is the hard cap
    deadline = time.monotonic() + timeout_secs

    try:
        proc = subprocess.Popen(
            cmd,
            cwd=ingest_script.parent,
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT,
            text=True,
            bufsize=1,
        )
    except Exception as e:
        print(f"    ✗ Failed to launch subprocess: {e}", flush=True)
        return 0, 1

    captured: List[str] = []
    timed_out = False
    try:
        assert proc.stdout is not None
        for line in proc.stdout:
            stripped = line.rstrip('\n')
            captured.append(stripped)
            print(f"    {stripped}", flush=True)
            if time.monotonic() > deadline:
                timed_out = True
                proc.kill()
                break
    except Exception as e:
        print(f"    ✗ Stream read error: {e}", flush=True)
        proc.kill()

    try:
        proc.wait(timeout=30)
    except subprocess.TimeoutExpired:
        proc.kill()
        proc.wait()

    if timed_out:
        print(f"    ✗ Pipeline timed out after {timeout_secs}s", flush=True)
        return 0, 1

    stdout_text = '\n'.join(captured)
    published = stdout_text.count('published')
    failed = stdout_text.count('[ERROR]') + stdout_text.count('[TERMINAL]')

    if proc.returncode != 0 and failed == 0:
        failed = 1

    return published, failed


def write_scan_log(scan_time: str, results: Dict[str, Dict]):
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_file = LOG_DIR / f"pipeline_run_{timestamp}.md"

    lines = [
        "# Pipeline Draft Watcher Scan Log\n",
        f"**Scan Time:** {scan_time}\n",
    ]

    for rtype, data in results.items():
        lines.append(f"\n## {rtype.upper()}\n")
        lines.append(f"- New files: {data.get('new_files', 0)}")
        lines.append(f"- Published: {data.get('published', 0)}")
        lines.append(f"- Failed: {data.get('failed', 0)}\n")

    lines.append(f"\n---\n*Generated by deprecated legacy draft watcher (BCE-732 archive)*\n")
    log_file.write_text('\n'.join(lines))
    print(f"\n✓ Scan log: {log_file}")
    return str(log_file)


def main():
    parser = argparse.ArgumentParser(description='DEPRECATED legacy draft watcher (BCE-732 archive)')
    parser.add_argument('--type', required=True, help='Report type (econ, mat, for, all)')
    parser.add_argument('--scan-only', action='store_true', help='Scan only')
    parser.add_argument('--dry-run', action='store_true', help='Test mode')
    parser.add_argument('--slug', type=str, help='Process specific slug only')
    parser.add_argument('--force', action='store_true', help='Force re-process')
    args = parser.parse_args()

    if not guard_legacy_watcher():
        return 2

    types = REPORT_TYPES if args.type == 'all' else [args.type]
    scan_time = datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')
    if not os.environ.get('GDRIVE_ROOT_FOLDER_ID'):
        print("ERROR: GDRIVE_ROOT_FOLDER_ID is required for draft scans.")
        return 1

    print("=" * 60)
    print(f"Pipeline Draft Watcher — BCE-732")
    print(f"Types: {', '.join(t.upper() for t in types)}")
    print(f"Scan Time: {scan_time}")
    print("=" * 60)

    results = {}
    for rtype in types:
        print(f"\n[SCAN] {rtype.upper()} drafts...")
        new_files = scan_type(rtype, slug=args.slug, force=args.force)
        print(f"  Found {len(new_files)} new file(s)")

        published, failed = 0, 0
        if new_files and not args.scan_only:
            published, failed = process_type(
                rtype,
                dry_run=args.dry_run,
                slug=args.slug,
                force=args.force,
            )

        results[rtype] = {
            'new_files': len(new_files),
            'published': published,
            'failed': failed,
        }

    log_file = write_scan_log(scan_time, results)

    print("\n" + "=" * 60)
    print("SCAN COMPLETE")
    for rtype, data in results.items():
        print(f"  {rtype.upper()}: {data['new_files']} new, {data['published']} published, {data['failed']} failed")
    print("=" * 60)

    total_failed = sum(d['failed'] for d in results.values())
    return 1 if total_failed > 0 else 0


if __name__ == '__main__':
    sys.exit(main())
