...
크레딧 소비:
  /v1/cryptocurrency/listings/latest (limit=500) = 3 credits
...
실행 모드:
  ...
  --mode top500   : 상위 500개만 수집 (3 credits)
...
Usage:
    ...
    python cmc_market_sync.py --mode top500 --dry-run
"""
...
def parse_cmc_rank(token: Dict) -> Optional[int]:
    """Return the CMC canonical rank when it is a positive integer."""
    try:
        rank = int(token.get('cmc_rank'))
...
def build_slug_map(tracked_projects: List[Dict], cmc_tokens: List[Dict]) -> Dict[str, str]:
    ...
    This helper is intentionally exact-match only. Canonical CMC market
    snapshots must not rewrite a CMC row to an arbitrary tracked project just
...
def mode_full(cmc: CMCClient, db: SupabaseClient, dry_run: bool = False) -> Dict:
    ...
    for token in tokens:
        cmc_slug = token.get('slug', '')
        # Deduplicate defensively: 같은 CMC slug이 여러 번 나올 경우
        # 시총이 높은 것(먼저 등장)을 우선
        if cmc_slug in seen_slugs:
            duplicates += 1
            continue
        seen_slugs.add(cmc_slug)
        rows.append(cmc_to_market_row(token))
...
def mode_tracked(cmc: CMCClient, db: SupabaseClient, dry_run: bool = False) -> Dict:
    ...
    상위 5,000개를 가져와서 tracked_projects와 매칭.
    추가로 매칭 안 된 프로젝트는 개별 처리 불필요 (대부분 top 5000 안에 있음).
    크레딧: 25 (5000 / 200)
    """
...
def mode_top500(cmc: CMCClient, db: SupabaseClient, dry_run: bool = False) -> Dict:
    """
    Quick sync: 상위 500개만 수집.
    크레딧: 3
    """
    print("\n=== CMC Market Sync: TOP 500 MODE ===")

    tokens = cmc.get_listings(start=1, limit=500)
    if not tokens:
        print("  [FAIL] No tokens fetched")
        return {'mode': 'top500', 'fetched': 0, 'written': 0}

    rows = []
    for canonical_rank, token in enumerate(tokens[:500], start=1):
        rows.append(cmc_to_market_row(
            token,
            cmc_rank_override=canonical_rank,
        ))

    print(f"  [CMC] {len(tokens)} tokens fetched, {len(rows)} canonical rank 1..500 rows prepared")
    print(f"  [CMC] Credits used: {cmc.credits_used}")
    ...
    return {'mode': 'top500', 'fetched': len(tokens), 'written': written,
            'credits': cmc.credits_used}
...
def main():
    ...
    parser.add_argument('--mode', choices=['full', 'tracked', 'top500'],
