#!/usr/bin/env python3
"""
Daily Pipeline Operations Report Generator — BCE-1870

파이프라인 운영 현황 일일 보고서를 생성하여 경영진에게 제공한다.

보고 항목:
- ECON/MAT/FOR 슬라이드 발행 상태
- 실패/blocked/manual/reconcile/stale 상태
- report type별 최근 24시간 처리 통계
- historical FOR tracker 격리 여부
...
SLIDE_PROCESSED_FILE = PIPELINE_DIR / 'output' / '_slide_processed.json'
LEGACY_FOR_PROCESSED_FILE = PIPELINE_DIR / 'output' / '_for_processed.json'
PROCESSED_FILE = SLIDE_PROCESSED_FILE
...
# Log directory
LOG_DIR = PIPELINE_DIR.parent.parent / 'logs' / 'slide_pipeline'
...
RECONCILE_STATUSES = {
    status for status in PAPERCLIP_SUCCESS_STATUSES | PAPERCLIP_BLOCKED_STATUSES
    if 'db_reconcile' in status
}
SUCCESS_STATUSES = PAPERCLIP_SUCCESS_STATUSES - RECONCILE_STATUSES
BLOCKED_STATUSES = PAPERCLIP_BLOCKED_STATUSES | {
...
def load_processed_data() -> Dict:
    """Load current slide watcher processed tracker."""
...
def load_legacy_for_processed_data() -> Dict:
    """Load historical FOR tracker for reproduction only."""
    if not LEGACY_FOR_PROCESSED_FILE.exists():
        return {}

    try:
        with open(LEGACY_FOR_PROCESSED_FILE, 'r', encoding='utf-8') as f:
            return json.load(f)
    except Exception as e:
        print(f"[WARN] Failed to load {LEGACY_FOR_PROCESSED_FILE}: {e}")
        return {}
...
def _parse_pipeline_timestamp(value: Optional[str]) -> Optional[datetime]:
    if not value:
        return None
    try:
...
def _record_updated_at(entry: Dict) -> datetime:
    for key in ('updated_at', 'finished_at', 'started_at', 'modifiedTime'):
        parsed = _parse_pipeline_timestamp(entry.get(key))
        if parsed:
            return parsed
    return datetime.now(timezone.utc)


def _entry_report_type(entry: Dict) -> str:
    rtype = str(entry.get('rtype') or entry.get('parent_folder_name') or '').lower()
    return rtype if rtype in REPORT_TYPES else 'unknown'


def _status_bucket(entry: Dict) -> str:
    status = entry.get('status') or 'unknown'
    if status == 'processing' and is_stale_processing(entry):
        return 'stale'
    if entry.get('stale_processing'):
        return 'stale'
    if status in RECONCILE_STATUSES or status.startswith(('db_reconcile_', 'dry_run_db_reconcile_')):
        return 'reconcile'
    if status in PAPERCLIP_FAILURE_STATUSES or status in FAILURE_CATEGORIES:
        return 'failed'
    if status in BLOCKED_STATUSES:
        return 'blocked'
    if status in SUCCESS_STATUSES or status == 'done':
        return 'success'
...
def summarize_slide_operations(processed_data: Dict, days: int = 1) -> Dict:
    """Summarize current watch_slides.py manifest records by report type and taxonomy."""
    cutoff_time = datetime.now(timezone.utc) - timedelta(days=days)
    by_type = {
        rtype: {
            'success': 0,
        ...
        for rtype in (*REPORT_TYPES, 'unknown')
    }
    totals = {
        'success': 0,
        'failed': 0,
    ...
    for file_id, entry in processed_data.items():
        updated_dt = _record_updated_at(entry)
        if updated_dt < cutoff_time:
            continue

        rtype = _entry_report_type(entry)
        bucket = _status_bucket(entry)
        status = entry.get('status') or 'unknown'
        row = by_type[rtype]
        row[bucket] += 1
        row['total'] += 1
        ...
        row['records'].append({
            ...
            'lang': entry.get('lang'),
            'name': entry.get('name') or entry.get('source_path') or file_id,
            'status': status,
            'bucket': bucket,
    ...
    for row in by_type.values():
        ...
        row['records'].sort(key=lambda record: record['updated_at'], reverse=True)
...
def generate_report(
    ...
    slide_summary: Optional[Dict] = None,
) -> str:
    ...
    slide_summary = slide_summary or {'by_type': {}, 'totals': {}}
    slide_totals = slide_summary.get('totals') or {}

    # Calculate statistics
    total_failures = int(slide_totals.get('failed', len(active_failures)))
    total_stale = int(slide_totals.get('stale', len(stale_processing)))
    total_successes = int(slide_totals.get('success', len(recent_successes)))
    total_blocked = int(slide_totals.get('blocked', 0))
    total_reconcile = int(slide_totals.get('reconcile', 0))
    total_processed = int(slide_totals.get('total', total_failures + total_successes))
...
**보고 일자**: {report_date}
**보고 기간**: {report_period}
**보고 대상**: Slide Publish Pipeline (ECON/MAT/FOR)
**운영 기준**: `watch_slides.py` + `_slide_processed.json` + Paperclip telemetry taxonomy
...
- **실패**: {total_failures}건
- **Blocked/Manual**: {total_blocked}건
- **Stale Processing**: {total_stale}건
- **DB Reconcile**: {total_reconcile}건
- **성공률**: {success_rate:.1f}%

"""
    by_type = slide_summary.get('by_type') or {}
    if by_type:
        report += """
## Report Type Status Matrix

| 유형 | 합계 | 성공 | 실패 | Blocked/Manual | Stale | DB Reconcile | 주요 상태 |
|------|------|------|------|----------------|-------|--------------|-----------|
"""
        for rtype in (*REPORT_TYPES, 'unknown'):
            ...
            top_statuses = ', '.join(
                f"{status}:{count}"
                for status, count in sorted(statuses.items(), key=lambda item: (-item[1], item[0]))[:5]
            )
            report += (
                f"| {rtype.upper()} | {row.get('total', 0)} | {row.get('success', 0)} | "
                f"{row.get('failed', 0)} | {row.get('blocked', 0)} | {row.get('stale', 0)} | "
                f"{row.get('reconcile', 0)} | {top_statuses or '-'} |\n"
            )
        report += "\n"
        ...
        for row in by_type.values():
            attention_records.extend(
                record for record in row.get('records', [])
                if record.get('bucket') in {'failed', 'blocked', 'stale', 'reconcile'}
            )
        if attention_records:
            report += """
## Operational Attention Queue

| 유형 | 상태군 | 상태 | 슬러그 | 언어 | 파일/경로 | 최근 갱신 | 오류 |
|------|--------|------|--------|------|-----------|-----------|------|
"""
            for record in sorted(attention_records, key=lambda item: item['updated_at'], reverse=True)[:30]:
                path = record.get('source_path') or record.get('name') or record.get('file_id')
                path_short = str(path)[:54] + ('...' if len(str(path)) > 54 else '')
                error = record.get('error') or ''
                error_short = str(error)[:60] + ('...' if len(str(error)) > 60 else '')
                report += (
                    f"| {record.get('rtype', '').upper()} | {record.get('bucket')} | "
                    f"{record.get('status')} | `{record.get('slug')}` | {record.get('lang') or '-'} | "
                    f"`{path_short}` | {record.get('updated_at', '')[:16].replace('T', ' ')} | "
                    f"{error_short or '-'} |\n"
                )
            report += "\n"
...
"""
    ...
    if not slide_summary and (total_failures > 0 or total_stale > 0):
        report += f"""
## Failures by Category
...
"""
    ...
    if not slide_summary and recent_successes:
        report += f"""
...
"""
    ...
    # Action items
    if total_failures > 0 or total_stale > 0 or total_blocked > 0 or total_reconcile > 0:
        report += f"""
---
...
"""
        if total_blocked > 0:
            report += f"""
### 2. Resolve Blocked/Manual States ({total_blocked}건)

`unresolved`, `mismatch`, `language_mismatch`, missing analysis source, prune skipped 계열은 `watch_slides.py`가 자동 발행을 멈춘 상태입니다. 파일명/본문 매칭, 누락 source, Supabase/Storage 연결 상태를 확인해야 합니다.

"""

        if total_reconcile > 0:
            report += f"""
### 3. Review DB Reconcile Outcomes ({total_reconcile}건)

`db_reconcile_*` 상태는 Drive `Slide/{{TYPE}}/`와 website-visible `project_reports`/`tracked_projects` 정합성 조정 결과입니다. cancelled/timestamp clear/sync 건은 운영 변경으로 간주해 표본 확인이 필요합니다.
...
**Pipeline**: Slide Publish Watcher (ECON/MAT/FOR)
...
**Legacy FOR Tracker**: `{LEGACY_FOR_PROCESSED_FILE.relative_to(PIPELINE_DIR.parent.parent)}` is historical/reproduction-only, not the current operations source of truth.
...
def main():
    ...
    print("=" * 70)
    print("Pipeline Operations Daily Report Generator — BCE-1870")
    ...
    legacy_for_data = load_legacy_for_processed_data()
    print(f"  ✓ Loaded {len(processed_data)} slide watcher manifest records")
    print(f"  ✓ Legacy FOR tracker records available for reproduction only: {len(legacy_for_data)}")

    # Step 2: Categorize failures
    print("\n[2/4] Analyzing failures...")
    slide_summary = summarize_slide_operations(processed_data, days=args.days)
    failures_by_category, active_failures, stale_processing, recent_successes = categorize_failures(
        processed_data, days=args.days
    )

    print(f"  ✓ Total records in window: {slide_summary['totals']['total']}")
    print(f"  ✓ Success: {slide_summary['totals']['success']}")
    print(f"  ✓ Failures: {slide_summary['totals']['failed']}")
    print(f"  ✓ Blocked/manual: {slide_summary['totals']['blocked']}")
    print(f"  ✓ Stale processing: {slide_summary['totals']['stale']}")
    print(f"  ✓ DB reconcile: {slide_summary['totals']['reconcile']}")
    ...
    print(f"  Total failures: {slide_summary['totals']['failed']}")
    print(f"  Blocked/manual: {slide_summary['totals']['blocked']}")
    print(f"  Stale processing: {slide_summary['totals']['stale']}")
    print("=" * 70)
