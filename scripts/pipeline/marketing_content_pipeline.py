...
ECON_SOURCE_FOLDER_ID = os.environ.get("BCE_MARKETING_ACTIVE_ECON_SOURCE_FOLDER_ID", "")
...
LEGACY_ECON_SOURCE_FOLDER_ID = os.environ.get(
    ...
    os.environ.get("BCE_MARKETING_ECON_SOURCE_FOLDER_ID", DEFAULT_ECON_SOURCE_FOLDER_ID),
) or DEFAULT_LEGACY_ECON_SOURCE_FOLDER_ID
DEFAULT_MAT_SOURCE_FOLDER_ID = "1-K4nMQcG3U-L1ro6YqBguCk2JUExlNcc"
MAT_SOURCE_FOLDER_ID = os.environ.get("BCE_MARKETING_ACTIVE_MAT_SOURCE_FOLDER_ID", "")
LEGACY_MAT_SOURCE_FOLDER_ID = os.environ.get(
    "BCE_MARKETING_LEGACY_MAT_SOURCE_FOLDER_ID",
    os.environ.get("BCE_MARKETING_MAT_SOURCE_FOLDER_ID", DEFAULT_MAT_SOURCE_FOLDER_ID),
)
DEFAULT_FOR_SOURCE_FOLDER_ID = "1eLyL9VSkwMM9TShea4Yd4WiveQuJfwKS"
FOR_SOURCE_FOLDER_ID = os.environ.get("BCE_MARKETING_ACTIVE_FOR_SOURCE_FOLDER_ID", "")
LEGACY_FOR_SOURCE_FOLDER_ID = os.environ.get(
    "BCE_MARKETING_LEGACY_FOR_SOURCE_FOLDER_ID",
    os.environ.get("BCE_MARKETING_FOR_SOURCE_FOLDER_ID", DEFAULT_FOR_SOURCE_FOLDER_ID),
)
ARCHIVE_FOLDER_ID = os.environ.get("BCE_MARKETING_ARCHIVE_FOLDER_ID", "")
ACTIVE_INGEST_PARENT_FOLDER_NAME = os.environ.get("BCE_ACTIVE_INGEST_PARENT_FOLDER_NAME", "BCE Lab Reports")
ACTIVE_INGEST_PARENT_FOLDER_ENV = "BCE_ACTIVE_INGEST_PARENT_FOLDER_ID"
ACTIVE_ANALYSIS_ROOT_FOLDER_NAME = os.environ.get("BCE_ACTIVE_ANALYSIS_ROOT_FOLDER_NAME", "analysis2")
...
def _normalize_source_scope(source_scope: Optional[str]) -> str:
    scope = (source_scope or "active").strip().lower()
    if scope not in DRIVE_SOURCE_SCOPES:
...
def _source_folder_id_for_report_type(report_type: str, *, source_scope: str = "active") -> str:
    ...
    if report_type == "for":
        raise RuntimeError(
            "BCE_MARKETING_ACTIVE_FOR_SOURCE_FOLDER_ID must point to analysis2/FOR "
            "or use source_scope=legacy for BCE_MARKETING_FOR_SOURCE_FOLDER_ID"
        )
    raise ValueError(f"No source folder configured for report_type={report_type} scope={source_scope}")
...
def _source_folder_ids_for_report_type(
    ...
    *,
    ...
    by_type = {
        "econ": {"active": active_ids.get("econ", ""), "legacy": LEGACY_ECON_SOURCE_FOLDER_ID},
        "mat": {"active": active_ids.get("mat", ""), "legacy": LEGACY_MAT_SOURCE_FOLDER_ID},
        "for": {"active": active_ids.get("for", ""), "legacy": LEGACY_FOR_SOURCE_FOLDER_ID},
    }
    if report_type not in by_type:
        raise ValueError(f"Unsupported report_type: {report_type}")
    scopes = ("active", "legacy") if scope == "all" else (scope,)
    return [by_type[report_type][item] for item in scopes if by_type[report_type][item]]
...
def _blocking_card_summary_reasons(reasons: Sequence[str]) -> List[str]:
    return [
        reason for reason in reasons
        if reason not in NON_BLOCKING_CARD_SUMMARY_REASONS
    ]
...
def _active_analysis_source_folder_ids(service) -> Dict[str, str]:
    ...
    if still_missing:
        print(
            "[WARN] active analysis2 ingest folders missing for "
...
def build_project_report_patch_from_drive_source(
    ...
    *,
    ...
    if blocking_reasons:
        raise RuntimeError(
            "Card summary quality gate failed for "
            f"{source.slug}/{source.report_type}: {', '.join(blocking_reasons)}"
        )
...
FORENSIC_RISK_SCORE_PATTERN = re.compile(
    r"(\d{1,3}(?:\.\d+)?)\s*/\s*100\s*(?:[-–—:]\s*)?"
    r"(CRITICAL|HIGH|ELEVATED|MODERATE|LOW|심각|높음|경계|보통|낮음|严重|高|警戒|中等)?",
    re.I,
)

FORENSIC_RISK_LEVEL_WORDS = {
    "critical": "critical",
    "심각": "critical",
    "严重": "critical",
    "high": "high",
...
def _extract_forensic_risk_metadata(source: MarkdownSource, content: DerivedContent) -> Dict[str, Any]:
    if source.report_type != "for":
        return {}

    candidates: List[str] = []
    candidates.extend(text for text in content.marketing_by_lang.values() if text)
    candidates.extend(text for text in content.summary_by_lang.values() if text)
    if source.text:
        candidates.append(source.text)

    for text in candidates:
        match = FORENSIC_RISK_SCORE_PATTERN.search(text)
        if not match:
            continue
        score = max(0, min(100, int(round(float(match.group(1))))))
        level_word = (match.group(2) or "").strip().lower()
        risk_level = FORENSIC_RISK_LEVEL_WORDS.get(level_word) or _risk_level_from_score(score)
        return {
            "risk_score": score,
...
def build_project_report_patch(
    ...
    *,
    ...
    forensic_risk = _extract_forensic_risk_metadata(source, content)
    if forensic_risk:
        card_data.update(forensic_risk)
    ...
    if forensic_risk:
        patch["risk_level"] = forensic_risk["risk_level"]
        patch["card_risk_score"] = forensic_risk["risk_score"]
...
def persist_content(sb, row: Dict[str, Any], source: MarkdownSource, content: DerivedContent, archived: Optional[Dict[str, Any]]) -> None:
    ...
    card_copy = derive_card_copy(source, project=project)
    blocking_reasons = _blocking_card_summary_reasons(card_copy.quality_reasons)
    if blocking_reasons:
        raise RuntimeError(
            "Card summary quality gate failed before persist for "
            f"{source.slug}/{source.report_type}: {', '.join(blocking_reasons)}"
        )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument(
        "--source-scope",
        choices=sorted(DRIVE_SOURCE_SCOPES),
        default="active",
        help="Drive analysis roots to scan when --source-folder-id is omitted.",
    )
    ...
    if args.source == "drive":
        if args.source_folder_id != SOURCE_FOLDER_ID:
            sources = load_drive_sources(args.source_folder_id)
        else:
            sources = []
            drive_service = _get_drive_service()
            report_types = [args.report_type] if args.report_type else ["econ", "mat", "for"]
