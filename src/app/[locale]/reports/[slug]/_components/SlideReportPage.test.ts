...
function mockReportQueries(
  project: unknown,
  reports: unknown[],
  aliasCandidates?: unknown[],
) {
  const projectQuery = createQuery(project, 'single')
  const aliasCandidatesQuery = createQuery(aliasCandidates ?? [project], 'order')
  const reportsQuery = createQuery(reports, 'order')
  let trackedProjectCallCount = 0

  mockCreateServerSupabaseClient.mockResolvedValue({
    from: jest.fn((table: string) => {
      if (table === 'tracked_projects') {
        trackedProjectCallCount += 1
        return trackedProjectCallCount === 1 ? projectQuery : aliasCandidatesQuery
      }
...
describe('SlideReportPage locale availability', () => {
  ...
  it('renders slide-coming-soon instead of locale-pending when the English Google Drive PDF exists without a slide URL', async () => {
    ...
    expect(screen.getByText('slideComingSoonTitle')).toBeTruthy()
    expect(screen.getByText('slideComingSoonDesc')).toBeTruthy()
  ...
  it('loads alias-matched reports for legacy project slugs so USD.AI report routes do not 404', async () => {
    mockReportQueries(
      { id: 'project-chip', slug: 'usd-ai', name: 'USD.AI', symbol: 'CHIP', cmc_id: 39870 },
      [
        {
          id: 'report-usdai-ko',
          project_id: 'project-usdai',
          language: 'ko',
          report_type: 'econ',
          status: 'published',
          version: 1,
          slide_html_urls_by_lang: {
            ko: 'https://example.test/usdai-ko.html',
          },
        },
      ],
      [
        { id: 'project-chip', slug: 'usd-ai', name: 'USD.AI', symbol: 'CHIP', coingecko_id: null, cmc_id: 39870, aliases: [] },
        { id: 'project-usdai', slug: 'usdai', name: 'USDai', symbol: 'USDAI', coingecko_id: 'usdai', cmc_id: null, aliases: ['USD.AI', 'usd.ai', 'usd ai', 'usdai'] },
      ],
    )

    const page = await SlideReportPage({ locale: 'ko', slug: 'usd-ai', reportType: 'econ' })
    render(page)

    expect(mockNotFound).not.toHaveBeenCalled()
    expect(screen.getByTestId('slide-viewer').getAttribute('data-url')).toBe(
      'https://example.test/usdai-ko.html',
    )
  })
})
