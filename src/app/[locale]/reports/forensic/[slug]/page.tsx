...
export default async function ForensicReportPage({ params }: Props) {
  ...
  const reportState = getLocaleReportState<ForensicReportRecord>(allRows, locale)
  if (reportState.status === 'not_found') notFound()

  const report =
    reportState.status === 'available'
      ? reportState.report
      : ((allRows && allRows.length > 0 ? allRows[0] : null) as ForensicReportRecord | null)
  ...
  const localizedPdfUrl = resolveReportPdfUrl(report, locale)
  ...
  const primaryUrl = localizedPdfUrl || report.file_url || report.gdrive_url
