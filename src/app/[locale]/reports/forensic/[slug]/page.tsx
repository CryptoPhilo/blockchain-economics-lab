...
import { createReportsRepository } from '@/lib/repositories/reports'
import {
  getReportVersionParam,
...
import {
  ...
  resolveSlideUrl,
} from '../../[slug]/_components/slide-report-utils'
...
type ForensicReportRecord = Record<string, any> & {
  id: string
  project_id: string
...
export default async function ForensicReportPage({ params, searchParams }: Props) {
  ...
  const requestedVersion = getReportVersionParam(version)
  const t = await getTranslations('forensicDetail')
  const supabase = await createServerSupabaseClient()
  const reportsRepository = createReportsRepository(supabase)
  const forensicResult = await reportsRepository.getForensicReportsBySlug(slug)

  if (!forensicResult) notFound()

  const { project, reports: allRows } = forensicResult

  const sortedRows = sortReportsLatestFirst((allRows || []) as ForensicReportRecord[])
  const effectiveLanguage = lang === locale ? lang : locale
  const localeRows = sortedRows.filter((row) => (
    row.language === effectiveLanguage || !row.language
  ))
  const requestedOrLatest = pickRequestedOrLatestReport(localeRows, {
    version: requestedVersion,
    language: effectiveLanguage,
  })
  const fallbackRequested = requestedVersion
    ? sortedRows.find((row) => Number(row.version || 0) === requestedVersion)
    : undefined
  const selectedAnchor = requestedOrLatest ?? fallbackRequested ?? sortedRows[0]
  const localizedVersionRows = selectedAnchor
    ? sortedRows.filter((row) => (
        row.version === selectedAnchor.version
        && (row.language === effectiveLanguage || !row.language)
      ))
    : []
  const versionRows = localizedVersionRows.length > 0
    ? localizedVersionRows
    : selectedAnchor
      ? sortedRows.filter((row) => row.version === selectedAnchor.version)
      : []
  const reportState = getLocaleReportState<ForensicReportRecord>(versionRows, locale)
  if (reportState.status === 'not_found') notFound()

  const report =
    reportState.status === 'available'
      ? reportState.report
      : ((versionRows && versionRows.length > 0 ? versionRows[0] : null) as ForensicReportRecord | null)
  ...
  const pageCount =
    typeof report.page_count === 'number' && report.page_count > 0 ? report.page_count : null
  ...
  const levelLabel =
    level === 'critical' ? t('riskCritical') : level === 'high' ? t('riskHigh') : t('riskElevated')
  ...
  for (const row of versionRows ?? []) {
    ...
    if (urls && typeof urls === 'object') {
      for (const [lang, value] of Object.entries(urls)) {
        if (typeof value === 'string' && value && !mergedSlideUrls[lang]) {
          mergedSlideUrls[lang] = value
        }
      }
    }
  }

  const localizedSlideUrl = resolveSlideUrl(mergedSlideUrls, locale)
  const previewStatusLabel = isComingSoon
    ? locale === 'ko'
      ? '준비 중'
      : 'Coming soon'
    : localizedSlideUrl
      ...
      : locale === 'ko'
        ? '슬라이드 준비 중'
        : 'Slide pending'
  const previewLengthLabel = pageCount
    ? `${pageCount}${locale === 'ko' ? '페이지' : ' pages'}`
    : locale === 'ko'
      ? '공개 범위 미표시'
      : 'Length not disclosed'
  const statusToneClass = isComingSoon
    ? 'text-amber-300'
    : localizedSlideUrl
      ? 'text-green-300'
      : 'text-gray-300'
  const previewCtaCopy = isComingSoon
    ? {
        ko: '포렌식 보고서는 공개 준비가 끝나는 대로 슬라이드 뷰어로 제공됩니다.',
        en: 'The forensic report will be available in the slide viewer once publishing is complete.',
      }
    : {
        ko: '슬라이드 뷰어가 준비되면 이 페이지에서 전체 보고서를 확인할 수 있습니다.',
        en: 'Once the slide viewer is ready, the full report will be available on this page.',
      }
  ...
  const accessHighlights =
    locale === 'ko'
      ? [
          ...
          isComingSoon
            ? '공개 전까지는 핵심 신호만 미리보기로 제공합니다.'
            : '슬라이드 뷰어가 준비되면 이 페이지에서 전체 내용을 제공합니다.',
        ]
      : [
          `Status: ${previewStatusLabel}`,
          `Report length: ${previewLengthLabel}`,
          isComingSoon
            ? 'Until publication, the preview focuses on the highest-signal findings.'
            : 'Once the slide viewer is ready, this page will provide the full report.',
        ]
  const reportCoverageTitle =
    locale === 'ko' ? '보고서에서 확인할 수 있는 내용' : 'What this report covers'
  ...
  const fallbackSignals = [
    ...
    change24h !== 0
      ...
      : null,
    pageCount ? (locale === 'ko' ? `${pageCount}페이지 보고서` : `${pageCount}-page report`) : null,
  ...
  return (
    <div className="min-h-screen">
      {/* Hero header */}
      <div
        className={`relative border-b ${config.border} bg-gradient-to-b from-gray-950 via-gray-950 to-transparent`}
      >
          ...
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-8">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-3">
                <span
                  className={`px-3 py-1 rounded-lg text-xs font-bold uppercase ${config.bg} ${config.color} border ${config.border}`}
                >
                  🔍 {t('forensic')}
                </span>
                <span
              ...
              {change24h !== 0 && (
                <p className="text-lg mb-4">
                  ...
                  <span
                    className={
                      change24h >= 0 ? 'text-green-400 font-semibold' : 'text-red-400 font-semibold'
                    }
                  >
                    {change24h >= 0 ? '+' : ''}
                    {Number(change24h).toFixed(1)}%{direction === 'up' ? ' ↑' : ' ↓'}
                  </span>
                </p>
              )}

              <p className="text-gray-400 text-lg leading-relaxed max-w-2xl">{summary}</p>
              {marketingContent && (
      ...
      <div className="max-w-5xl mx-auto px-6 py-12">
          ...
          <div className="mb-10 rounded-[28px] border border-white/10 bg-white/[0.03] p-6 md:p-8">
            <div className="grid gap-6 lg:grid-cols-[minmax(0,1.45fr)_minmax(320px,0.9fr)]">
              ...
              <aside className="rounded-[24px] border border-indigo-400/20 bg-[linear-gradient(160deg,rgba(99,102,241,0.18),rgba(17,24,39,0.92))] p-5">
                ...
                <h3 className="mt-3 text-2xl font-bold text-white">{accessPanelTitle}</h3>
                <p className="mt-2 text-sm leading-6 text-indigo-100/85">{accessPanelBody}</p>
                ...
                <div className="mt-5 rounded-2xl border border-white/10 bg-black/20 p-4 text-sm text-gray-200">
                  <p>
                    {isComingSoon
                      ? t('comingSoonDesc')
                      : locale === 'ko'
                        ? '슬라이드 뷰어가 준비되면 이 페이지에 표시됩니다.'
                        : 'The slide viewer will appear on this page when it is ready.'}
                  </p>
                </div>
              </aside>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-10">
          <div className="rounded-xl bg-white/[0.03] border border-white/5 p-5">
            ...
            <p className="text-white font-semibold">{t('reportTypeName')}</p>
          </div>
          <div className="rounded-xl bg-white/[0.03] border border-white/5 p-5">
        ...
        <div className="rounded-xl bg-yellow-500/5 border border-yellow-500/20 p-6">
          <p className="text-xs text-yellow-600/80 leading-relaxed">⚠ {t('disclaimer')}</p>
        </div>
