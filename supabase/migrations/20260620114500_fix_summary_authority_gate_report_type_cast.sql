...
COMMENT ON FUNCTION public.promote_report_summary_job(uuid, text, text, text) IS
  'Atomically promotes a validation-passed report_summary_jobs row into project_reports with audit/event writes and promotion lock handling. Casts project_reports.report_type enum to text for report_summary_jobs compatibility.';
