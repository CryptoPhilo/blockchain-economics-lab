...
COMMENT ON FUNCTION public.promote_report_summary_job(uuid, text, text, text) IS
  'Atomically promotes a validation-passed report_summary_jobs row into website-visible project_reports language siblings with audit/event writes and promotion lock handling.';
