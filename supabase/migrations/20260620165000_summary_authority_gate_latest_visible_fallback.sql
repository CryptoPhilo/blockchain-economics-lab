...
COMMENT ON FUNCTION public.promote_report_summary_job(uuid, text, text, text) IS
  'Atomically promotes a validation-passed report_summary_jobs row into website-visible project_reports language siblings, falling back to the latest visible report version when a stale candidate source version has no visible target.';
