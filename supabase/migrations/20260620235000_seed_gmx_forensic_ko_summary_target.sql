-- BCE-2036: create the missing Korean FOR target shell for GMX.
--
-- The Summary Authority Gate promotes a validation-passed Korean candidate into
-- a website-visible project_reports row for the same project/report/locale. GMX
-- currently has only a version-1 forensic English coming_soon shell, so the
-- gate correctly blocks on gmx/forensic/ko. This migration seeds the matching
-- Korean shell without publishing a slide asset.
...
SELECT
  ...
  COALESCE(sr.trigger_data, '{}'::jsonb)
    || jsonb_build_object('seeded_by', 'BCE-2036', 'source_report_id', sr.id),
  sr.risk_level,
  sr.file_url,
  sr.page_count,
  sr.task_id,
  sr.product_id,
  sr.title_en,
  COALESCE(sr.title_ko, tp.name, 'GMX'),
  sr.title_fr,
  sr.title_es,
  sr.title_de,
  sr.title_ja,
