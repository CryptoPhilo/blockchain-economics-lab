-- Create the missing Jupiter Korean forensic report shell required by the
...
SELECT
  ...
  jsonb_build_object(
    ...
    'summary_authority_target', jsonb_build_object(
      'issue', 'BCE-2462',
      'blocked_issue', 'BCE-2461',
      'job_id', 'eb1cc5dc-f4e6-460a-97ae-76fee0457a97',
      'source_identity', 'drive:1Ln3S1XuJerQcKpRT2PLpQaDnqBgv0no6:0B8HYgThT3NByNzlUYlRrL3AzOWxKR2l3dEJlNkpZSXFlbVU0PQ',
      'drive_file_id', '1Ln3S1XuJerQcKpRT2PLpQaDnqBgv0no6',
      'revision_id', '0B8HYgThT3NByNzlUYlRrL3AzOWxKR2l3dEJlNkpZSXFlbVU0PQ',
      'created_for', 'Summary Authority Gate target backfill'
    )
  ),
  true,
  'summary-authority-target:jupiter/forensic/ko/version:1',
  'Jupiter 시장 무결성 및 심층 포렌식 리스크 보고서.md',
  now(),
  now()
FROM target_project
...
WHERE target_project.slug = 'jupiter'
