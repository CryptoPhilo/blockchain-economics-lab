-- Create the missing Dohrnii Korean forensic report shell required by the
...
SELECT
  ...
  'ko',
  NULL,
  'Dohrnii Forensic Report',
  'Dohrnii 포렌식 리스크 분석 보고서',
  ...
  jsonb_build_object(
    ...
    'summary_authority_target', jsonb_build_object(
      'issue', 'BCE-2506',
      'blocked_issue', 'BCE-2505',
      'job_id', 'ce638c20-be69-4d7c-a7de-81fd15036031',
      'source_identity', 'drive:1S97JlkEwHJcV2-ndadHlPuYfda9KyPw-:0B8HYgThT3NBybjZGWTFxeUlKNmFNUmRZbWxYcjVpV0RWNzhVPQ',
      'drive_file_id', '1S97JlkEwHJcV2-ndadHlPuYfda9KyPw-',
      'revision_id', '0B8HYgThT3NBybjZGWTFxeUlKNmFNUmRZbWxYcjVpV0RWNzhVPQ',
      'source_sha256', '336d2bc529d3fae678285587d85d7e3db47aa5774d0aad3b3dc6e92740ce51bc',
      'created_for', 'Summary Authority Gate target backfill'
    )
  ),
  true,
  'summary-authority-target:dohrnii/forensic/ko/version:1',
  'Dohrnii(DHN) 시장 무결성 및 심층 포렌식 리스크 보고서.md',
  now(),
  now()
FROM target_project
