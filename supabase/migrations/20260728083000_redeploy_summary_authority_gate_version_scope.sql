...
BEGIN
  ...
  WHERE project_id = v_project_id
    ...
    AND (v_candidate_version IS NULL OR version = v_candidate_version)
  LIMIT 1
  FOR UPDATE;

  IF NOT FOUND THEN
    RAISE EXCEPTION
      'website-visible project_reports target not found: %/%/% version %',
      v_job.project_slug,
      v_job.report_type,
      COALESCE(v_job.locale, 'ko'),
      COALESCE(v_candidate_version::text, '<unspecified>');
  END IF;

  UPDATE public.project_reports
  SET
    card_data =
      COALESCE(card_data, '{}'::jsonb)
...
COMMENT ON FUNCTION public.promote_report_summary_job(uuid, text, text, text) IS
  'Atomically promotes a validation-passed report_summary_jobs row into same-version website-visible project_reports language siblings with audit/event writes and promotion lock handling.';
